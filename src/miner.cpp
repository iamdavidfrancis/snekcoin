...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
...

    {
        ...
        txNew.vout[0].nValue = nFees + GetSnekcoinBlockSubsidy(nHeight, consensus, pindexPrev->GetBlockHash());
...
static bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("SnekcoinMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("SnekcoinMiner: ProcessNewBlock, block not accepted");
...
{
    LogPrintf("SnekcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("snekcoin-miner");
    const CChainParams& chainparams = Params();
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in SnekcoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("Running SnekcoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("SnekcoinMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SnekcoinMiner runtime error: %s\n", e.what());
        return;
    }
}
