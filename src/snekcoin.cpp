...
// Snekcoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateSnekcoinNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
...
CAmount GetSnekcoinBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, uint256 prevHash)
{
...
int64_t GetSnekcoinDustFee(const std::vector<CTxOut> &vout, CFeeRate &baseFeeRate) {
    int64_t nFee = 0;
